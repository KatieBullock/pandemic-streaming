...
async function addSearchContent() {
  try {
    const searchResults = await searchMulti();
...
async function addSearchCarouselEvents() {
  try {
    ...
    document.querySelector("#search-forward").addEventListener("click", () => {
      if (searchItems[displayCount] && searchItems[displayCount + 1]) {
        ...
        if (searchItems[displayCount - 1] && searchItems[displayCount - 2]) {
...
export { searchMulti, addSearchContent, addSearchCarouselEvents };
